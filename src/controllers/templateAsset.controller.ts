  ...
  try {
    ...
    const data = await Promise.all(
      assets.map(async (a: any) => ({
        ...a,
        composite_image_url: await s3Service.presignIfS3(a.composite_image_url),
      })),
    )
...
export async function generateAsset(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!celebrity) { res.status(404).json({ success: false, message: 'Celebrity not found' }); return }
    if (!template.background_image_url) {
      res.status(400).json({ success: false, message: 'Template has no background image' }); return
    }
    if (!celebrity.thumbnail_url) {
      res.status(400).json({ success: false, message: 'Celebrity has no thumbnail image' }); return
    }
...
export async function updateAssetTtsModel(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!ttsModel || !VALID_MODELS.includes(ttsModel)) {
      res.status(400).json({ success: false, message: 'Invalid ttsModel value' }); return
    }
